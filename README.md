...
Deploy the contract LetterCredit. As inputs insert the address of the buyer and of the seller. 
The fintech address is the one that deploys the contract. 

Then, switch to the buyer address: it is time to send a document that explicitates the files and details required in
order to consider the transaction successfull and a deadline for the seller (function `buyerUpload()` with as input the hash of the 
...
comply. 
The result is determined by majority principle, and in the event of a tie we decide to set the documents to not compliant. 
